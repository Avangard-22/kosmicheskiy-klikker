...
function init() {
    ...
    loadDailyBonusData();    injectStyles();
    setupEventHandlers();
    checkDailyBonus();
    startTimer();
...
function saveDailyBonusData() {
    try {
        ...
        console.log('💾 [BONUS] Сохранено в localStorage:', dailyBonusData);
    } catch (e) {
...
function updateWelcomeButton() {
    ...
    if (isAvailable) {
        btn.classList.add('available');        btn.classList.remove('claimed');
    } else {
...
function startTimer() {
    console.log('⏱️ [BONUS] Запуск таймера...');
    if (timerInterval) clearInterval(timerInterval);
    
    timerInterval = setInterval(() => {
        console.log('🔄 [BONUS] updateTimerDisplay вызван');
        updateTimerDisplay();
    }, 1000);
    
    console.log('✅ [BONUS] Таймер запущен');
}

/**
 * ⏱️ Обновление таймера обратного отсчёта
 * ✅ ИСПРАВЛЕНО: таймер обновляется ВСЕГДА, даже когда бонус доступен
 */
function updateTimerDisplay() {
    const timerEl = document.getElementById('dailyBonusTimer');
    if (!timerEl) {
        console.error('❌ [BONUS] dailyBonusTimer не найден в DOM');
        return;
    }
    ...
    console.log('🔍 [BONUS] lastClaimDate:', dailyBonusData.lastClaimDate);
    console.log('🔍 [BONUS] today:', today);
    console.log('🔍 [BONUS] isAvailable:', isAvailable);
    console.log('🔍 [BONUS] currentDay:', dailyBonusData.currentDay);

    if (dailyBonusData.currentDay > 30) {
        timerEl.textContent = '🎉 Завершено';
        timerEl.style.color = '#FFD700';
        console.log('🎉 [BONUS] Цикл завершён');
        return;
    }
    ...
    if (isAvailable) {
        ...
        console.log('✅ [BONUS] Бонус доступен');
        return;    }

    // ✅ Бонус уже получен сегодня - показываем таймер до следующего дня
    console.log('⏱️ [BONUS] Расчёт таймера...');
    ...
    console.log('⏱️ [BONUS] Таймер обновлён:', timeString);
}
...
function injectStyles() {
    ...
    style.textContent = `
        ...
        .daily-bonus-welcome-btn:hover {            transform: translateY(-3px);
...
function setupEventHandlers() {
    ...
    if (welcomeBtn) {
        welcomeBtn.addEventListener('click', showModal);        welcomeBtn.addEventListener('touchstart', (e) => {
            e.preventDefault();
            showModal();
        }, { passive: false });
    }
...
function applyReward(reward) {
    ...
    switch (reward.type) {
        case 'crystals':
            ...
            break;
        case 'boost':            const duration = window.shopSystem?.config?.[reward.boost]?.duration || getBoostDuration(reward.boost);
...
function showNotification(text, color) {
    ...
    notif.style.cssText = `
        ...
        padding: 15px 25px; border-radius: 12px; z-index: 4000;        text-align: center; font-family: 'Orbitron', sans-serif; font-weight: bold;
