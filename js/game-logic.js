...
const astronomicalUnits = { mercury: 0.38710, venus: 0.72333, earth: 1.00000, mars: 1.52366, jupiter: 5.20336, saturn: 9.53707, uranus: 19.19126, neptune: 30.06896, pluto: 39.48200 };
...
function pauseGame() {
    ...
    // ✅ Гарантируем плавное возобновление анимации
    const shopPanel = document.getElementById('shopPanel');
    if (shopPanel && window.getComputedStyle(shopPanel).maxHeight === 'none') {
        shopPanel.style.maxHeight = '65vh';
        shopPanel.style.overflowY = 'auto';
        shopPanel.style.scrollbarWidth = 'thin';
        shopPanel.style.scrollbarColor = '#4CAF50 #222';
    }
...
// ✅ Безопасное получение множителей из магазина
function getShopMultiplier(type, fallback = 1) {
    if (window.shopSystem && typeof window.shopSystem[type] === 'function') {
        return window.shopSystem[type]();
    }
    // Фолбэк на старые состояния, если магазин ещё не инициализирован
    if (window.gameState?.shopItems) {
        const ids = type === 'getSpeedMultiplier' ? 'timeWarp' : type === 'getDamageMultiplier' ? 'powerSurge' : 'crystalBoost';
        if (window.gameState.shopItems[ids]?.active) return 1.5; // Примерная совместимость
    }
    return fallback;
}
...
function getCurrentSpeed() {
    ...
    speed *= getShopMultiplier('getSpeedMultiplier', 1);
    return speed;
}
...
function calculateBlockHealth() {
    ...
    const randomFactor = balanceConfig.healthRandomRange.min + Math.random() * (balanceConfig.healthRandomRange.max - balanceConfig.healthRandomRange.min);
    return Math.floor(combinedHealth * randomFactor * getShopMultiplier('getBlockHealthMultiplier', 1));
}

function calculateClickPower() {
    const basePower = 1;
    const upgradeBonus = window.gameState.clickUpgradeLevel;
    const diminishingEffect = Math.pow(balanceConfig.damageProgression.diminishingReturns, Math.min(window.gameState.clickUpgradeLevel, balanceConfig.damageProgression.maxLevelEffect));
...
function getRareBlockType() {
    ...
    const luckMult = getShopMultiplier('getLuckMultiplier', 1);
    for (const [type, block] of Object.entries(rareBlocks)) {
        cumulativeChance += block.chance * luckMult;
...
function announceRareBlock(blockName) {
    ...
    setTimeout(() => { if (announce.parentNode) document.body.removeChild(announce); }, 2000);
}
...
function updateUpgradeButtons() {
    ...
    // ✅ Обновлённая логика для Bobo: Циклы 1-10, множитель цены x1.4, Счётчик в тултипе
    const activations = window.gameState.helperActivations || 0;
    const cycle = Math.floor(activations / 10); // 0, 1, 2...
    const progressInCycle = (activations % 10) + 1; // 1-10
    
    // Цена растет каждые 10 активаций с множителем 1.4
    const dynamicHelperCost = Math.floor(baseHelperUpgradeCost * Math.pow(1.4, cycle));
    
    const upgradeHelperBtn = document.getElementById('upgradeHelperBtn');
    if (upgradeHelperBtn) {
        const isActive = window.gameState.helperActive;
        
        // ✅ Убрали надпись "АКТИВЕН", показываем цену или прочерк
        upgradeHelperBtn.querySelector('.upgrade-cost').textContent = isActive ? '...' : dynamicHelperCost.toLocaleString();
        
        if (isActive) {
            upgradeHelperBtn.className = 'upgrade-btn btn-unavailable';
            upgradeHelperBtn.title = `Bobo работает... Осталось: ${Math.ceil((window.gameState.helperTimeLeft||0)/1000)}с`;
        } else {
            const canAfford = window.gameState.coins >= dynamicHelperCost;
            upgradeHelperBtn.className = canAfford ? 'upgrade-btn btn-available' : 'upgrade-btn btn-unavailable';
            // ✅ Счётчик активаций 1-10 и информация о цикле в тултипе
            upgradeHelperBtn.title = `Активация: ${progressInCycle}/10 | Цикл: ${cycle + 1} | Множитель цены: x${Math.pow(1.4, cycle).toFixed(1)}`;
        }
    }
...
function checkLocationUpgrade() {
    ...
    if (nextLocation && currentAU >= targetAU) {
        ...
        const tooltipText = window.formatString(window.translations[window.currentLanguage].locationProgress.unlocked, { location: locations[nextLocation].name });
...
function setLocation(loc) {
    ...
    if (window.planetBackground && window.planetBackground.setPlanet) {
    ...
    if (levelAnnounce) {
        ...
        setTimeout(() => { levelAnnounce.style.opacity = "0"; }, 2000);
    }
...
function applyUpgradePenalty() {
    ...
    if (getShopMultiplier('isInvincible', false)) {
...
function showComboText(combo, bonus, block) {
    ...
    text.textContent = window.formatString(window.translations[window.currentLanguage].tooltips.combo, { count: combo, bonus: bonus });
...
function showRewardText(reward, block) {
    ...
    text.textContent = window.formatString(window.translations[window.currentLanguage].tooltips.reward, { reward: reward });
...
function hitBlock(block, damage) {
    if (!window.gameState || !window.gameState.gameActive || isGamePaused) return;
    ...
    // ✅ Динамический расчёт урона с бонусами магазина
    let finalDamage = damage * getShopMultiplier('getDamageMultiplier', 1);
    let isCrit = false;
    
    // ✅ Динамический расчёт крита
    let critChance = window.gameState.critChance * getShopMultiplier('getCritChanceMultiplier', 1);
    let critMult = window.gameState.critMultiplier * getShopMultiplier('getCritMultMultiplier', 1);
...
function destroyBlock(block) {
    ...
    const randomBonus = balanceConfig.randomBonusRange.min + Math.random() * (balanceConfig.randomBonusRange.max - balanceConfig.randomBonusRange.min);
    ...
    // ✅ Множитель награды из магазина
    reward = Math.floor(reward * getShopMultiplier('getRewardMultiplier', 1));
    ...
    if (window.gameState.comboCount > 1) {
        let comboMult = balanceConfig.comboMultiplier * getShopMultiplier('getComboMultiplier', 1);
        const comboBonus = Math.floor(reward * (window.gameState.comboCount * comboMult));
        reward += comboBonus;
...
function animateBlock(block) {
    ...
    function move() {
        if (isGamePaused || !window.gameState?.gameActive || currentBlock !== block) {
            requestAnimationFrame(move); // Продолжаем цикл, но не двигаем блок
            return;
        }
...
// ✅ ИСПРАВЛЕНА ПОЗИЦИЯ BOBO
function moveHelperToRandomPosition() {
    if (!helperElement) return;
    
    // Определяем цель (блок)
    let target = currentBlock ? currentBlock.getBoundingClientRect() : { left: window.innerWidth/2, top: window.innerHeight/2, width: 0, height: 0 };
    
    // Границы поиска (отступы от краев)
    const margin = 60;
    ...
    while (!found && attempts < 30) {
        ...
        const ry = margin + Math.random() * (maxY - margin); // Отступ сверху тоже
        
        const dist = Math.hypot(rx - (target.left + target.width/2), ry - (target.top + target.height/2));
        
        // Проверяем дистанцию (не слишком близко, не слишком далеко)
        if (dist > 130 && dist < 280) {
            helperPosition = { x: rx, y: ry };
            found = true;
        }
        attempts++;
    }
    
    // Если не нашли случайную позицию, ставим в правый нижний угол
    if (!found) {
        helperPosition = { x: window.innerWidth - 100, y: window.innerHeight - 100 };
    }
...
function createHelperElement() {
    ...
    helperElement.style.position = 'fixed'; // ✅ Фиксируем относительно окна, чтобы не улетал за пределы скролла
    helperElement.style.zIndex = '9998';
...
function activateHelper() {
    ...
    // ✅ Проверка: если Bobo уже активен, ничего не делаем
    if (window.gameState.helperActive) {
        if (window.showTooltip) window.showTooltip("Bobo уже работает!");
        return;
    }
    ...
    helperInterval = setInterval(() => {
        if (window.gameState && window.gameState.helperActive && currentBlock && window.gameState.gameActive && !isGamePaused) {
            helperAttack();
        }
    }, 1500);
...
function helperAttack() {
    if (!currentBlock || !window.gameState || !window.gameState.helperActive || !helperElement || isGamePaused) return;
    ...
    // ✅ Множитель урона из магазина
    let finalHelperDmg = upgradedHelperDmg * getShopMultiplier('getDamageMultiplier', 1);
...
function buyHelper() {
    ...
    // ✅ Проверка: если Bobo уже активен, не даем купить
    if (window.gameState.helperActive) {
        if (window.showTooltip) window.showTooltip('Bobo уже активен!');
        return;
    }

    // ✅ Расчет стоимости с учетом цикла активаций (x1.4 каждые 10 активаций)
    const cycle = Math.floor((window.gameState.helperActivations || 0) / 10);
    const finalCost = Math.floor(baseHelperUpgradeCost * Math.pow(1.4, cycle));
...
function continueGame() {
    ...
    if (window.loadGame()) {
        ...
        if (window.showTooltip) {
            const tooltipText = window.formatString('Игра загружена! Кристаллы: {coins}, Урон: {damage}', {
