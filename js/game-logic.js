  ...
  function updateUpgradeButtons() {
    ...
    if (upgradeHelperBtn) {
      ...
      if (window.gameState.coins >= helperCost && !window.gameState.helperActive) {
        ...
        upgradeHelperBtn.title = "Активировать помощника Bobo";
      } else {
        upgradeHelperBtn.className = "upgrade-btn btn-unavailable";
        upgradeHelperBtn.title = window.gameState.helperActive ? "Помощник уже активен" : "Недостаточно кристаллов";
      }
    }
...
Сможешь побить мой рекорд?`;
  ...
  function buyClickPower() {
    ...
    if (window.gameState.coins >= cost) {
      ...
      // Эффект недостаточно кристаллов
      const upgradeBtn = document.getElementById('upgradeClickBtn');
      if (upgradeBtn) {
  ...
  function buyHelper() {
    ...
    if (window.gameState.coins >= cost && !window.gameState.helperActive) {
      ...
      updateUpgradeButtons();
      window.saveGame();
    } else if (window.gameState.helperActive) {
      if (window.showTooltip) {
        window.showTooltip(window.translations[window.currentLanguage].tooltips.helperAvailable);
        setTimeout(window.hideTooltip, 1500);
      }
  ...
  // Обновляем метрики только если это новая игра
  if (reset) {
    ...
    // При продолжении только обновляем время начала сессии
    window.gameMetrics.startTime = Date.now();
  }
 ...
 function continueGame() {
  ...
  if (window.loadGame()) {
    ...
    // Обновляем HUD перед стартом
    updateHUD();
    updateUpgradeButtons();
    updateProgressBar();
    setLocation(window.gameState.currentLocation);
    
    // Запускаем игру без сброса
    startGame(false);
...
if (continueBtn) {
  ...
  continueBtn.addEventListener('touchstart', (e) => {
    e.preventDefault();
    console.log('🔄 Проверка сохранения (touch)...');
    const hasSave = localStorage.getItem('cosmicBlocksSave') !== null;
