...
let helperTimer = null; // ✅ Глобальный таймер для возможности перезапуска

// Защита от отсутствия gameState
...
// Применяем замедление из магазина
...
// ✅ УБРАНА ПРОВЕРКА !window.gameState.helperActive
if (window.gameState.coins >= helperCost) {
upgradeHelperBtn.className = "upgrade-btn btn-available";
upgradeHelperBtn.title = "Активировать помощника Bobo на 1 минуту";
} else {
...
// 🔒 Запрет возврата к предыдущим локациям
...
if (!window.gameState) return;
// ✅ Если уже активен — перезапускаем
...
activateHelper(); // ✅ Теперь можно нанимать многократно
updateHUD();
updateUpgradeButtons();
window.saveGame();
} else {
...
// ✅ Сброс состояния помощника при старте игры
window.gameState.helperActive = false;
